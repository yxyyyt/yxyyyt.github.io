...
循环边，选择一条边，向某一个方向延伸三条表，除掉两条同面边，另外一条就是相邻循环边。
...
shift+鼠标左键+点、线、面，可以选择多个模式；

面模式下，鼠标左键单击选择一个面，双击选择多个面；
...
选择多个面，i，内插会一起影响多个面；再按 i，各个面独立内插；



### 倒角

边线倒角 ctrl + b，移动鼠标控制倒角宽度，滚轮控制分段数，形状控制斜切（以 0.25 为单位）;
...
环切 ctrl + r，鼠标滚轮控制切割次数

偏移环切边，shift+ctrl+r，类似于对面上的线作倒角



### 切割

切割 k 自动吸附点或边；按住 shift 自动到达边的中点位置

切分 shift+空格 -> shift+8，可以切除掉内侧或外侧



### 多边形建形

shift+空格 -> shift+9，ctrl+鼠标左键设置多边形一点



### 旋绕

对齐点位，选择点+g+y，ctrl+选择另外一个点

沿轴心点旋绕（坐标位置）

- 到顶视图，alt+e，选择旋绕
- shift+空格 -> shift+0，注意法线方向（开启视图叠加层的面朝向）
